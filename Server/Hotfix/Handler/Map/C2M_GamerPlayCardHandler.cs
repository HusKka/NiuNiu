...
namespace ETHotfix
{
    ...
    public class C2M_GamerPlayCardHandler : AMActorRpcHandler<Gamer, C2M_GamerPlayCard, M2C_GamerPlayCard>
    {
        protected override async Task Run(Gamer gamer, C2M_GamerPlayCard message, Action<M2C_GamerPlayCard> reply)
        {
            M2C_GamerPlayCard response = new M2C_GamerPlayCard();
            try
            {
                ...
                //转发玩家出牌消息
                room.Broadcast(new M2C_GamerPlayCard_Ntt() { UserID = gamer.UserID, Cards = message.Cards });
